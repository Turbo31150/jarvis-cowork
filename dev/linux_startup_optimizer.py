    ...
    try:
        ...
        for line in out.strip().split("\n")[1:]:
            if "logon" in line.lower() or "At log on" in line:
                ...
                if len(parts) >= 2:
                    name = parts[0].strip('"')
                    status = parts[2].strip('"') if len(
                        parts) > 2 else "Unknown"
                    impact, rec = classify_impact(name, "")
                    items.append({"name": name,
                                  "command": "",
                                  "location": "Task Scheduler",
                                  "source": "schtasks_logon",
                                  "impact": impact,
                                  "recommendation": rec,
...
def do_enable(name):
    ...
    db.execute(
        ...
        (now.isoformat(),
         ...
         name,
         0,
         "Enable requires original command path. Check actions log for previously disabled items."),
    )
    db.commit()
...
def do_status():
    ...
    result = {
        ...
        "script": "win_startup_optimizer.py",
        "script_id": 253,
        "db": str(DB_PATH),
...
def main():
    parser = argparse.ArgumentParser(
        description="win_startup_optimizer.py — Startup optimization (#253)")
    parser.add_argument(
        "--scan",
        action="store_true",
        help="Full startup scan")
    parser.add_argument(
        "--disable",
        type=str,
        metavar="NAME",
        help="Disable a startup item")
    parser.add_argument(
        "--enable",
        type=str,
        metavar="NAME",
        help="Enable a startup item")
    parser.add_argument(
        "--impact",
        action="store_true",
