...
def get_latest_report():
    ...
    if not os.path.exists(REPORT_DIR):
        return None
    reports = sorted([f for f in os.listdir(
        REPORT_DIR) if f.endswith(".json")])
    if not reports:
        return None
...
def format_report_email(report):
    ...
    lines.append(f"  RAM: {sys_info.get('ram_free_gb',
                                        '?')}GB libre / {sys_info.get('ram_total_gb',
                                                                      '?')}GB")
...
def send_weekly_summary(recipient=RECIPIENT):
    ...
    reports = sorted([f for f in os.listdir(
        REPORT_DIR) if f.endswith(".json")])[-7:]
    if not reports:
    ...
    for rf in reports:
        ...
        try:
            ...
            online = sum(1 for v in cluster.values()
                         if v.get("status") == "OK")
            sys_info = data.get("system", {})
            lines.append(f"{rf[:15]}: {online} nodes | CPU {sys_info.get('cpu_pct',
                                                                         '?')}% | RAM {sys_info.get('ram_free_gb',
                                                                                                    '?')}GB free")
...
if __name__ == "__main__":
    if "--once" in sys.argv:
        ...
        cmd = [
            ...
            "JARVIS Test Email",
            "--body",
            f"Test email from JARVIS Report Mailer at {
        ...
        result = subprocess.run(
            ...
            text=True,
            timeout=30)
        print("Test email:", "OK" if result.returncode ==
              0 else result.stderr[:200])
