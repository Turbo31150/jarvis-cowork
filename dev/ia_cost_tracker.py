        ...
        try:
            ...
            for t in [t[0] for t in db.execute(
                ...
                if "metric" in t.lower():
                    ...
                    if "node" in cols:
                        for r in db.execute(
                            f"SELECT node FROM [{t}] WHERE ts > ?",
                            (time.time() - 86400,
                             )).fetchall():
                            # est ~500 tokens/req
                            usage[r[0] or "unknown"] += 500
            db.close()
        except Exception:
            pass
    costs = []
    for model, tokens in usage.items():
        rate = next((v for k, v in COST_RATES.items()
                    if k.lower() in model.lower()), 0)
        cost = tokens * rate / 1000
        costs.append({"model": model, "tokens": tokens,
...
def do_track():
    ...
    for c in costs:
        db.execute(
            "INSERT INTO costs (ts, model, tokens, cost_usd) VALUES (?,?,?,?)",
            (time.time(),
             c["model"],
                c["tokens"],
                c["cost_usd"]))
    ...
    db.execute(
        "INSERT INTO daily_reports (ts, date, total_tokens, total_cost, over_budget) VALUES (?,?,?,?,?)",
        (time.time(),
