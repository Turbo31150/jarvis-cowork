...
def print_summary(results: List[Dict], total_time: float) -> None:
    ...
    errors = status_counts.get("error",
                               0) + status_counts.get("not-found",
                                                      0) + status_counts.get("permission-denied",
                                                                             0)
    ...
    if failures:
        ...
        for r in failures:
            print(
                f"  [{r['status']:>10s}] {r['script']} -- {r['error'] or 'unknown'}")
...
def run_category(
    ...
    print_header(
        f"{cat_name.upper()} ({count} scripts) -- priority {CATEGORY_PRIORITY.get(cat_name, '?')}")
    ...
    if selected_categories:
        ...
        for cat_name in selected_categories:
            ...
            if cat_name not in categories:
                print(
                    f"ERROR: Unknown category '{cat_name}'. Available: {
                        ', '.join(
                            CATEGORY_PRIORITY.keys())}")
                return 1
    ...
    if json_output:
        report = {
            ...
            "summary": {
                ...
                "errors": sum(
                    1 for r in all_results if r["status"] in (
                        "error",
                        "not-found",
                        "permission-denied")),
            },
