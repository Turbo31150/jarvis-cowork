#!/usr/bin/env python3
"""dispatch_ops_runner.py -- Dynamic runner for uncabled dispatch_*.py scripts.

Scans the cowork/dev directory for dispatch_*.py scripts not already wired
individually in the autonomous orchestrator, then runs each one sequentially
with --once, logging results to cowork_gaps.db.

Usage:
    python dispatch_ops_runner.py --once          # Run all uncabled dispatch scripts
    python dispatch_ops_runner.py --list          # List discovered scripts
    python dispatch_ops_runner.py --dry-run       # Show plan without executing
...
TABLE_NAME = "dispatch_runner_log"
TIMEOUT = 60
...
ALREADY_WIRED = {
    "dispatch_quality_tracker.py",
    "dispatch_learner.py",
    "dispatch_error_analyzer.py",
    "dispatch_quick_bench.py",
    "dispatch_auto_improve.py",
    "dispatch_ab_tester.py",
    "dispatch_quality_scorer.py",
}
...
def discover_scripts():
    """Find all dispatch_*.py scripts excluding self and already-wired ones."""
    scripts = []
