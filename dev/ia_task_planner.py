...
def do_execute():
    ...
    db = init_db()
    goals = db.execute(
        "SELECT id, goal FROM goals WHERE status='planned' LIMIT 1").fetchall()
    ...
    for step_id, num, desc, command in steps:
        ...
        results.append({"step": num, "desc": desc,
                       "ok": ok, "output": output[:200]})
        if not ok:
            all_ok = False
            break
...
def get_status():
    ...
    db = init_db()
    goals = db.execute(
        "SELECT id, goal, status, ts FROM goals ORDER BY ts DESC LIMIT 10").fetchall()
    db.close()
    return [{"id": g[0], "goal": g[1], "status": g[2], "ts": datetime.fromtimestamp(
        g[3]).isoformat() if g[3] else None} for g in goals]


def main():
    parser = argparse.ArgumentParser(
        description="IA Task Planner — Autonomous goal decomposition")
    parser.add_argument(
        "--once",
        action="store_true",
        help="Execute pending goals")
    parser.add_argument(
        "--plan",
        metavar="GOAL",
        help="Create a plan for goal")
    parser.add_argument(
        "--execute",
        action="store_true",
        help="Execute pending plans")
    parser.add_argument(
        "--status",
        action="store_true",
        help="Show goals status")
