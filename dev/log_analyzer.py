"""Log Analyzer — COWORK auto-generated docstring."""
...
def analyze_log(path, include_errors=False, detect_trends=False):
    ...
    if include_errors:
        errors = [
            e for e in entries if e['level'].upper() in (
                'ERROR', 'CRITICAL', 'FAIL')]
        result['errors'] = [{'timestamp': e['timestamp'],
                             'message': e['message']} for e in errors]
    if detect_trends:
        # Simple trend: count occurrences of words in error messages over time
        # (by hour)
        ...
        trend_summary = {
            hour: cnt.most_common(5) for hour,
            cnt in trend.items()}
...
def main():
    ...
    parser.add_argument(
        'logfile',
        help='Chemin vers le fichier .log à analyser')
    parser.add_argument(
        '--analyze',
        action='store_true',
        help='Effectuer l\'analyse (par défaut)')
    parser.add_argument(
        '--errors',
        action='store_true',
        help='Inclure les erreurs dans le rapport')
    parser.add_argument(
        '--trends',
        action='store_true',
        help='Détecter les tendances d\'erreurs')
    parser.add_argument(
        '--report',
        action='store_true',
        help='Sortie du rapport au format JSON')
    ...
    try:
        ...
        if args.report:
            ...
            if args.trends:
                print(
                    f"Detected trends for {len(data.get('trends', {}))} time buckets.")
