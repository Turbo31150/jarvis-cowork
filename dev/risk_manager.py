...
def init_db():
    ...
    c.execute(
        "INSERT OR IGNORE INTO config (key, value) VALUES ('capital', ?)",
        (str(DEFAULT_CAPITAL),
         ))
    conn.commit()
    return conn
...
def check_trade_approval(conn, symbol, size, direction="LONG"):
    ...
    c = conn.cursor()
    c.execute(
        "INSERT INTO risk_log (ts, action, symbol, size, risk_pct, approved, reason) VALUES (?,?,?,?,?,?,?)",
...
def record_trade_result(conn, pnl, won):
    ...
    c = conn.cursor()
    c.execute(
        """INSERT INTO daily_pnl (date, pnl, trades, wins) VALUES (?, ?, 1, ?)
        ...
        (today,
         ...
         int(won)))
    capital = get_capital(conn)
    c.execute("UPDATE config SET value=? WHERE key='capital'",
              (str(round(capital + pnl, 4)),))
    conn.commit()
...
def show_status(conn):
    ...
    print(
        f"  Risk/trade: {RISK_PER_TRADE_PCT}% = ${round(capital * RISK_PER_TRADE_PCT / 100, 2)}")
...
if __name__ == "__main__":
    ...
    if "--status" in sys.argv:
        ...
        c = conn.cursor()
        c.execute(
            "SELECT ts, action, symbol, size, approved, reason FROM risk_log ORDER BY id DESC LIMIT 15")
        for r in c.fetchall():
            print(
                f"  {r[0][:16]} [{'OK' if r[4] else 'NO'}] {r[1]} {r[2]} ${r[3]} — {r[5][:60]}")
    else:
