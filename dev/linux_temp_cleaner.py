...
TEMP_DIRS = [
    {"name": "System_Temp", "path": "/tmp"},
    {"name": "User_Cache", "path": os.path.expanduser("~/.cache")},
    {"name": "Apt_Archives", "path": "/var/cache/apt/archives"},
    {"name": "User_Logs", "path": os.path.expanduser("~/jarvis/data/logs")},
]

SAFE_EXTENSIONS = {".tmp", ".log", ".bak", ".old", ".dmp", ".gz", ".cache"}
MAX_AGE_DAYS = 7
            ...
            try:
                if item.is_file():
                    ...
                    if mtime < cutoff:
                        ...
                        if ext in SAFE_EXTENSIONS or dir_info["name"] in (
                                "TEMP", "Windows_Temp"):
                            cleanable_files += 1
                            cleanable_size += size
            ...
            for item in path.iterdir():
                try:
                    if item.is_file() and item.stat().st_mtime < cutoff:
                        ext = item.suffix.lower()
                        if ext in SAFE_EXTENSIONS or d["name"] in (
                                "TEMP", "Windows_Temp"):
                            size = item.stat().st_size
                            item.unlink()
                            deleted += 1
...
def main():
    ...
    parser.add_argument(
        "--schedule",
        action="store_true",
        help="Schedule cleaning")
    parser.add_argument(
        "--stats",
        action="store_true",
        help="Cleaning history")
