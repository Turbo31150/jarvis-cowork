...
'''
    try:
        result = subprocess.run(
            ["bash", "-ExecutionPolicy", "Bypass", "-Command", ps_script],
...
def listen_stdin():
    ...
    print(
        'Format: {"level":"info|warning|critical","source":"module","message":"text"}')
    for line in sys.stdin:
        ...
        try:
            ...
            print(
                f"[{datetime.now().strftime('%H:%M:%S')}] {'OK' if ok else 'FAIL'}: {source} — {message[:60]}")
...
if __name__ == "__main__":
    if "--test" in sys.argv:
        ok = show_toast(
            "JARVIS Test", f"Notification OK — {
                datetime.now().strftime('%H:%M:%S')}")
        print(f"Toast: {'OK' if ok else 'FAIL'}")
    elif "--alert" in sys.argv:
        level = sys.argv[sys.argv.index(
            "--alert") + 1] if len(sys.argv) > sys.argv.index("--alert") + 1 else "info"
        msg = " ".join(sys.argv[sys.argv.index(
            "--alert") + 2:]) or "Test alert"
