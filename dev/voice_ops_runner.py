...
Scans the cowork/dev directory for voice_*.py scripts not already wired
individually in the autonomous orchestrator, then runs each one sequentially
with --once, logging results to cowork_gaps.db.

Usage:
    python voice_ops_runner.py --once          # Run all uncabled voice scripts
    python voice_ops_runner.py --list          # List discovered scripts
    python voice_ops_runner.py --dry-run       # Show plan without executing
...
ALREADY_WIRED = {
    "voice_auto_learner.py",
    "voice_correction_miner.py",
}
...
def discover_scripts():
    """Find all voice_*.py scripts excluding self and already-wired ones."""
    scripts = []
