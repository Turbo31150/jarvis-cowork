...
#!/usr/bin/env python3
"""List Bluetooth devices via PowerShell Get-PnpDevice -Class Bluetooth, output as JSON."""
...
def query_bluetooth_devices():
    """Run PowerShell to get Bluetooth devices."""
    ps_cmd = (
    ...
    if not output:
        return {
            "devices": [],
            "count": 0,
            "note": "No Bluetooth devices found or Bluetooth not available"}
...
def main():
    parser = argparse.ArgumentParser(
        description="List Bluetooth devices via PowerShell (JSON output)")
    parser.add_argument(
        "--once",
        action="store_true",
