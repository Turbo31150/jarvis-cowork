...
def _run_curl(command: list[str]) -> str:
    ...
    """
    proc = subprocess.run(["curl.exe", *command],
...
def load_model(name: str, node: str | None = None) -> dict:
    ...
    if node:
        ...
        _run_curl(["-X", "POST", "-H", "Content-Type: application/json",
                  "-d", payload, f"{LM_STUDIO_URL}/v1/load"])
    else:
        # Ollama – pull le modèle si nécessaire
        _run_curl(["-X", "POST", "-H", "Content-Type: application/json",
                  "-d", f'{{"name":"{name}"}}', f"{OLLAMA_URL}/api/pull"])
    return {
        "model": name,
        "action": "load",
        "node": node or "ollama",
...
def unload_model(name: str, node: str | None = None) -> dict:
    ...
    if node:
        payload = json.dumps({"model": name, "node": node})
        _run_curl(["-X", "POST", "-H", "Content-Type: application/json",
                  "-d", payload, f"{LM_STUDIO_URL}/v1/unload"])
    else:
...
def _prune_expired() -> None:
    ...
    expired = [
        name for name,
        ts in _model_cache.items() if now -
        ts > timedelta(
            seconds=MODEL_TTL_SECONDS)]
    for name in expired:
        unload_model(name)
...
def main() -> None:
    parser = argparse.ArgumentParser(
        description="Gestionnaire de modèles IA (LM Studio + Ollama)")
    ...
    group.add_argument(
        ...
        metavar=(
            "OLD",
            "NEW"),
        help="Swap du modèle OLD vers NEW")
    group.add_argument(
        "--optimize",
        action="store_true",
        help="Optimiser l'usage de la VRAM")
    parser.add_argument(
        "--node",
        help="Nom du nœud cible (LM Studio). Si absent, Ollama est utilisé.")
