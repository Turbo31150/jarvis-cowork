    ...
    try:
        ...
        for node in ast.walk(tree):
            if isinstance(node, ast.Call):
                ...
                if isinstance(
                        func,
                        ast.Attribute) and func.attr == "add_argument":
                    arg_names = []
                    help_text = ""
                    for arg in node.args:
                        if isinstance(
                                arg, ast.Constant) and isinstance(
                                arg.value, str):
...
def scan_scripts(verbose=False):
    ...
    for filepath in sorted(glob.glob(pattern)):
        ...
        info = {
            ...
            "line_count": lines,
            "status": status,
            "title": docstring.split("\n")[0].strip() if docstring else name.replace(
                ".py",
                "").replace(
                "_",
                " ").title()}
        ...
        if verbose:
            print(
                f"  Scanned: {name} ({lines} lines, status={status}, {
                    len(cli_args)} args)")
...
def generate_markdown(scripts):
    ...
    for s in scripts:
        ...
        if s["cli_args"]:
            ...
            lines.append("")
        lines.append(
            f"- **Status:** {s['status']} | **Lines:** {s['line_count']}")
        lines.append("")

    lines.append("---")
    lines.append(
        f"_Auto-generated by `auto_documentation_updater.py` on {now}_")
    lines.append("")
