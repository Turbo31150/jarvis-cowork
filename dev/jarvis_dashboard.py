...
def build_dashboard():
    ...
    nodes = safe_query(
        gaps,
        "SELECT node, last_status, consecutive_fails FROM heartbeat_state ORDER BY node")
    data["nodes"] = {
        r["node"]: {
            "status": r["last_status"],
            "fails": r["consecutive_fails"]} for r in nodes}
    data["nodes_online"] = sum(
        1 for r in nodes if r["last_status"] == "online")
    data["nodes_total"] = len(nodes)

    # 2. Reliability scores
    rel = safe_query(
        gaps,
        "SELECT node, composite, rank FROM node_reliability ORDER BY rank")
    data["reliability"] = {
        r["node"]: {
            "score": r["composite"],
            "rank": r["rank"]} for r in rel}

    # 3. Circuit breakers
    cb = safe_query(
        gaps,
        "SELECT node, state, fail_count FROM circuit_breaker_state")
    data["circuit_breakers"] = {
        r["node"]: {
            "state": r["state"],
            "fails": r["fail_count"]} for r in cb}

    # 4. Load
    load = safe_query(
        gaps,
        "SELECT node, current_load, total_dispatched, avg_response_ms FROM node_load")
    data["load"] = {
        r["node"]: {
            "current": r["current_load"],
            "total": r["total_dispatched"],
            "avg_ms": round(
                r["avg_response_ms"])} for r in load}

    # 5. Latency baselines
    lat = safe_query(
        gaps,
        "SELECT node, avg_ms, p95_ms, sample_count FROM latency_baselines")
    data["latency"] = {
        r["node"]: {
            "avg_ms": round(
                r["avg_ms"]), "p95_ms": round(
                r["p95_ms"]), "samples": r["sample_count"]} for r in lat}

    # 6. Dispatch stats
    try:
        has = edb.execute(
            "SELECT COUNT(*) FROM sqlite_master WHERE name='agent_dispatch_log'").fetchone()[0]
        if has:
            row = edb.execute("""
            ...
            data["dispatch"] = {
                "total": row["total"],
                "success_pct": row["rate"] or 0,
                "quality_pct": row["quality"] or 0,
                "avg_lat_ms": row["avg_lat"] or 0}
            total_all = edb.execute(
    ...
    if data["orchestrator"]["total_runs"] > 0:
        data["orchestrator"]["success_pct"] = round(
            (data["orchestrator"]["total_runs"] -
             data["orchestrator"]["total_fails"]) /
            data["orchestrator"]["total_runs"] *
            100,
            1)
...
def format_dashboard(data):
    ...
    lines.append(
        f"  Cluster   {data['nodes_online']}/{data['nodes_total']} online  {node_str}")
    ...
    if data.get("risks"):
        risk_str = " ".join(
            f"{r['target']}({r['score']:.0f})" for r in data["risks"][:3])
        lines.append(
            f"  Risks     {len(data['risks'])} items  max={data['max_risk']:.0f}  {risk_str}")
    else:
    ...
    lines.append(
        f"  Scripts   {
            data['scripts']}  DB: gaps={
            data['db']['gaps_mb']}MB etoile={
                data['db']['etoile_mb']}MB")
