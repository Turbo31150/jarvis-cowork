...
def list_plans() -> List[Dict[str, str]]:
    ...
    for line in out.splitlines():
        # Exemple de ligne : "Power Scheme GUID:
        # 8c5e7fda-e8bf-4a96-9a85-a6e23a8c635c  (Balanced) *"
        m = re.search(
            r"Power Scheme GUID:\s*([0-9a-fA-F-]+)\s*\(([^)]+)\)(?:\s*\*)?",
            line)
        if m:
            guid, name = m.groups()
            active = "*" in line
...
def set_active_plan(identifier: str) -> Dict[str, str]:
    ...
    guid = None
    for p in plans:
        if p["guid"].lower() == identifier.lower(
        ) or p["name"].lower() == identifier.lower():
            guid = p["guid"]
            break
    if not guid:
...
def get_battery_info() -> Dict[str, Any]:
    ...
    try:
        out = run_cmd(["wmic", "PATH", "Win32_Battery", "Get",
        ...
        if len(lines) >= 2:
            ...
            status = int(values[1]) if len(
                values) > 1 and values[1].isdigit() else None
...
def get_sleep_settings() -> Dict[str, Any]:
    ...
    for line in out.splitlines():
        if "SUB_SLEEP" in line:
            ...
            continue
        m = re.search(
            r"Power Setting GUID: ([0-9a-fA-F-]+)\s+\(([^)]+)\)\s+\[([^]]+)\]",
            line)
        if m:
...
def main():
    parser = argparse.ArgumentParser(
        description="Gestionnaire de plans d'alimentation Windows pour le cluster IA")
    group = parser.add_mutually_exclusive_group()
    group.add_argument(
        "--status",
        action="store_true",
        help="Affiche le plan actif et ses paramètres")
    group.add_argument(
        "--plan",
        action="store_true",
        help="Liste tous les plans disponibles")
    group.add_argument(
        "--set-plan",
        metavar="NAME_OR_GUID",
        help="Définit le plan actif")
    group.add_argument(
        "--battery",
        action="store_true",
        help="Affiche l'état de la batterie")
    group.add_argument(
        "--sleep-settings",
        action="store_true",
        help="Affiche les paramètres de mise en veille")
    args = parser.parse_args()

    try:
        if args.status:
            active = get_active_plan()
            print(json.dumps({"active_plan": active},
            ...
            print(json.dumps({"new_active_plan": new},
                  ensure_ascii=False, indent=2))
        elif args.battery:
