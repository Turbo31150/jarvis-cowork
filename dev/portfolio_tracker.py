...
def add_position(conn, symbol, direction, entry_price, quantity, leverage=10):
    c = conn.cursor()
    c.execute(
        "INSERT INTO positions (symbol, direction, entry_price, quantity, leverage, opened_at) VALUES (?,?,?,?,?,?)",
        (symbol,
         direction,
         entry_price,
         quantity,
    ...
    conn.commit()
    print(
        f"Position added: {direction} {symbol} @ {entry_price} x{quantity} (lev {leverage}x)")


def close_position(conn, pos_id, exit_price):
    c = conn.cursor()
    c.execute(
        "SELECT symbol, direction, entry_price, quantity, leverage FROM positions WHERE id=? AND status='open'",
        (pos_id,
         ))
    row = c.fetchone()
    if not row:
    ...
    c.execute(
        "UPDATE positions SET status='closed', closed_at=?, exit_price=?, pnl=? WHERE id=?",
        (datetime.now().isoformat(),
         exit_price,
         round(
            pnl,
...
def portfolio_snapshot(conn):
    ...
    c = conn.cursor()
    c.execute(
        "SELECT id, symbol, direction, entry_price, quantity, leverage FROM positions WHERE status='open'")
    ...
    for pos_id, symbol, direction, entry, qty, lev in positions:
        ...
        arrow = "+" if pnl >= 0 else ""
        lines.append(
            f"  #{pos_id} {direction} {symbol}: ${current} ({arrow}{
                round(
                    pnl_pct,
                    2)}%) P&L: ${
                round(
    ...
    # Save snapshot
    c.execute("INSERT INTO snapshots (ts, total_value, unrealized_pnl, open_positions, data) VALUES (?,?,?,?,?)", (datetime.now(
    ).isoformat(), total_pnl, total_pnl, len(positions), json.dumps({"prices": {k: v for k, v in prices.items()}})))
    conn.commit()
...
def show_history(conn, limit=10):
    c = conn.cursor()
    c.execute(
        "SELECT ts, unrealized_pnl, open_positions FROM snapshots ORDER BY id DESC LIMIT ?",
        (limit,
         ))
    rows = c.fetchall()
...
if __name__ == "__main__":
    ...
    if "--once" in sys.argv:
        ...
        c = conn.cursor()
        c.execute(
            "SELECT id, symbol, direction, entry_price, quantity, leverage, opened_at FROM positions WHERE status='open'")
        rows = c.fetchall()
        print(f"[OPEN POSITIONS] {len(rows)}")
        for r in rows:
            print(
                f"  #{r[0]} {r[2]} {r[1]} @ {r[3]} x{r[4]} lev={r[5]} ({r[6][:16]})")
    else:
        print("Usage: portfolio_tracker.py --once | --loop | --history | --positions")
        print(
            "       --add SYMBOL DIRECTION ENTRY [QTY] | --close ID EXIT_PRICE")
