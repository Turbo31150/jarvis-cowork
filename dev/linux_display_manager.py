...
def get_display_info():
    """Get display info using ctypes user32 and bash."""
    monitors = []
    try:
...
def get_brightness():
    """Get brightness via bash WMI."""
    try:
        cmd = 'bash -NoProfile -Command "try { (Get-CimInstance -Namespace root/WMI -ClassName WmiMonitorBrightness).CurrentBrightness } catch { Write-Output -1 }"'
        r = subprocess.run(
            cmd,
            capture_output=True,
            text=True,
...
def set_brightness(level):
    """Set brightness via bash WMI."""
    level = max(0, min(100, level))
    try:
        cmd = f'bash -NoProfile -Command "try {{ (Get-CimInstance -Namespace root/WMI -ClassName WmiMonitorBrightnessMethods).WmiSetBrightness(1, {level}) ; Write-Output OK }} catch {{ Write-Output FAIL }}"'
        r = subprocess.run(
            cmd,
            capture_output=True,
            text=True,
...
def set_resolution(res_str):
    """Set resolution via bash (requires restart or display settings)."""
    try:
        parts = res_str.lower().split("x")
        w, h = int(parts[0]), int(parts[1])
        # Use bash Set-DisplayResolution or QRes approach
        cmd = f'bash -NoProfile -Command "Set-DisplayResolution -Width {w} -Height {h} -Force 2>$null; if($?) {{ Write-Output OK }} else {{ Write-Output NOTAVAIL }}"'
        r = subprocess.run(
            cmd,
            capture_output=True,
            text=True,
...
def rotate_display():
    ...
    try:
        cmd = 'bash -NoProfile -Command "Get-CimInstance -ClassName Win32_VideoController | Select-Object Name, VideoModeDescription, CurrentRefreshRate | ConvertTo-Json"'
        r = subprocess.run(
            cmd,
            capture_output=True,
            text=True,
