...
def analyze():
    ...
    rows = edb.execute(
        "SELECT classified_type,node,latency_ms,success FROM agent_dispatch_log WHERE latency_ms>0 ORDER BY id DESC LIMIT 2000").fetchall()
    edb.close()
    g = {}
    for r in rows:
    ...
    for (pat, node), d in g.items():
        ...
        if fr > 0.3 and p95 > base * 1000:
            rec = min(rec * 1.5, MAX_TO)
        rec = max(MIN_TO, min(rec, MAX_TO))
        recs.append({"pattern": pat,
                     "node": node,
                     "samples": len(d["l"]),
                     "ok_pct": round((1 - fr) * 100,
                                     1),
                     "p50": round(p50),
                     "p95": round(p95),
                     "max": round(mx),
    ...
    return {"timestamp": datetime.now().isoformat(),
            "combos": len(recs),
            "avg_timeout": round(sum(r["timeout_s"] for r in recs) / max(len(recs),
                                                                         1),
                                 1),
            "problems": probs[:10],
...
def apply_cfg(recs):
    ...
    for r in recs:
        c.execute(
            "INSERT INTO timeout_configs (timestamp,pattern,node,recommended_timeout_s,p50_latency_ms,p95_latency_ms,max_latency_ms,sample_count,applied) VALUES (?,?,?,?,?,?,?,?,1)",
            (ts,
             r["pattern"],
                r["node"],
                r["timeout_s"],
...
def main():
    ...
    a = p.parse_args()
    if not any([a.once, a.apply, a.stats]):
        p.print_help()
        sys.exit(1)
    if a.stats:
        c = get_db()
        rows = c.execute(
            "SELECT pattern,node,recommended_timeout_s,p95_latency_ms,timestamp FROM timeout_configs ORDER BY id DESC LIMIT 20").fetchall()
        c.close()
        ...
        if "error" in data:
            ...
            return
        if a.apply:
            data["applied"] = apply_cfg(data["recommendations"])
        result = data
