...
def log_event(db, action, printer=None, details=None, success=1):
    db.execute(
        "INSERT INTO printer_events (ts, action, printer, details, success) VALUES (?,?,?,?,?)",
...
def get_printers():
    """Get printer list via bash."""
    try:
        cmd = 'bash -NoProfile -Command "Get-Printer | Select-Object Name, PrinterStatus, Type, DriverName, PortName, Shared, Published | ConvertTo-Json"'
        r = subprocess.run(
            cmd,
            capture_output=True,
            text=True,
...
def get_default_printer():
    """Get default printer."""
    try:
        cmd = 'bash -NoProfile -Command "(Get-CimInstance -ClassName Win32_Printer | Where-Object {$_.Default -eq $true}).Name"'
        r = subprocess.run(
            cmd,
            capture_output=True,
            text=True,
...
def get_print_queue():
    ...
    try:
        cmd = 'bash -NoProfile -Command "Get-PrintJob -PrinterName * -ErrorAction SilentlyContinue | Select-Object Id, PrinterName, DocumentName, UserName, SubmittedTime, JobStatus, Size | ConvertTo-Json"'
        r = subprocess.run(
            cmd,
            capture_output=True,
            text=True,
...
def set_default_printer(name):
    ...
    try:
        ...
        cmd = f'bash -NoProfile -Command "Set-DefaultPrinter -Name \\"{safe_name}\\" -ErrorAction SilentlyContinue; if($?) {{ Write-Output OK }} else {{ (New-Object -ComObject WScript.Network).SetDefaultPrinter(\\"{safe_name}\\"); Write-Output OK2 }}"'
        r = subprocess.run(
            cmd,
            capture_output=True,
            text=True,
...
def do_list():
    ...
    db.execute(
        "INSERT INTO printer_snapshots (ts, printers_json, total_printers, default_printer) VALUES (?,?,?,?)",
        (datetime.now().isoformat(),
         json.dumps(printers),
         len(printers),
         default))
...
def do_once():
    ...
    total_events = db.execute(
        "SELECT COUNT(*) FROM printer_events").fetchone()[0]
    issues = [p for p in printers if p.get("status_code", 0) not in [0]]
...
def main():
    parser = argparse.ArgumentParser(
        description="Windows Printer Manager — COWORK #229")
    parser.add_argument(
        "--list",
        action="store_true",
