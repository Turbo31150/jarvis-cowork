...
def add_message(conn, role, content, source="telegram", topic=None):
    c = conn.cursor()
    c.execute(
        "INSERT INTO conversations (ts, role, content, source, topic) VALUES (?, ?, ?, ?, ?)",
...
def add_fact(conn, category, fact, confidence=1.0, source=None):
    c = conn.cursor()
    c.execute(
        "INSERT INTO facts (category, fact, confidence, source, created) VALUES (?, ?, ?, ?, ?)",
        (category,
         fact,
         confidence,
         source,
...
def get_facts(conn, category=None, limit=20):
    ...
    if category:
        c.execute(
            "SELECT category, fact, confidence FROM facts WHERE category = ? ORDER BY created DESC LIMIT ?",
            (category,
             limit))
    else:
        c.execute(
            "SELECT category, fact, confidence FROM facts ORDER BY created DESC LIMIT ?",
            (limit,
             ))
    return [{"category": r[0], "fact": r[1], "confidence": r[2]}
            for r in c.fetchall()]
...
def track_topic(conn, topic):
    c = conn.cursor()
    c.execute(
        """INSERT INTO topics (topic, last_mentioned, mention_count) VALUES (?, ?, 1)
...
def get_recent_context(conn, limit=10):
    c = conn.cursor()
    c.execute(
        "SELECT ts, role, content, source FROM conversations ORDER BY id DESC LIMIT ?",
        (limit,
         ))
    return [{"ts": r[0], "role": r[1], "content": r[2], "source": r[3]}
...
def get_hot_topics(conn, limit=5):
    c = conn.cursor()
    c.execute(
        "SELECT topic, mention_count, last_mentioned FROM topics ORDER BY mention_count DESC LIMIT ?",
        (limit,
         ))
...
if __name__ == "__main__":
    ...
    if "--stats" in sys.argv:
        ...
        cat = sys.argv[sys.argv.index(
            "--facts") + 1] if len(sys.argv) > sys.argv.index("--facts") + 1 else None
        facts = get_facts(conn, category=cat)
        print(f"[CONTEXT ENGINE] {len(facts)} facts" +
              (f" [{cat}]" if cat else ""))
        for f in facts:
        ...
        print(
            "Usage: context_engine.py --stats | --context | --facts [category]")
