...
DEFAULT_PROFILES = {
    "dev": [
        ...
        "LM Studio"],
    "trading": [
        "chrome --new-window https://www.mexc.com",
        "chrome --new-window https://www.tradingview.com"],
    "morning": [
        "chrome",
        "outlook"],
    "meeting": [
...
def do_status():
    ...
    profiles = db.execute(
        "SELECT name, apps, auto_hour FROM profiles ORDER BY name").fetchall()
...
def main():
    ...
    parser.add_argument(
        "--launch",
        metavar="CONTEXT",
        help="Launch context profile")
    parser.add_argument(
        "--learn",
        action="store_true",
        help="Learn from usage")
