        ...
        try:
            ...
            for t in [t[0] for t in db.execute(
                ...
                if "metric" in t.lower():
                    try:
                        for r in db.execute(
                            f"SELECT node, status, latency_ms FROM [{t}] WHERE ts > ?",
                            (time.time() - 604800,
                             )).fetchall():
                            n = r[0] or "unknown"
...
def calibrate():
    ...
    for agent, w in CURRENT_WEIGHTS.items():
        s = next((v for k, v in stats.items()
                 if agent.lower() in k.lower()), None)
        if not s or (s["ok"] + s["fail"]) < 3:
            continue
        total = s["ok"] + s["fail"]
        sr = s["ok"] / total
        al = s["lat"] / total
        ...
        results.append({"agent": agent, "current": w, "suggested": nw, "sr": round(
            sr, 3), "lat": round(al, 1), "delta": round(nw - w, 2)})
        db.execute(
            "INSERT INTO calibrations (ts, agent, current_weight, new_weight, success_rate, avg_latency) VALUES (?,?,?,?,?,?)",
            (time.time(),
             agent,
             w,
             nw,
