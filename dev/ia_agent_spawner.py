...
def spawn_agent(db, spec):
    ...
    if not script:
        db.execute(
            "UPDATE agents SET status='error', stderr='No script specified' WHERE id=?",
            (agent_id,
             ))
        db.commit()
        return {
    ...
    try:
        ...
        try:
            ...
            db.execute(
                ...
                (agent_id,
                 "warning",
                 f"Timeout after {timeout}s, killed PID {
                     proc.pid}"))
        db.commit()
        row = db.execute("SELECT * FROM agents WHERE id=?",
                         (agent_id,)).fetchone()
        cols = [
...
def list_agents(db, limit=20):
    ...
    for st in [
        ...
        "error",
            "pending"]:
        cnt = db.execute(
            "SELECT COUNT(*) FROM agents WHERE status=?", (st,)).fetchone()[0]
        if cnt > 0:
            stats[st] = cnt
...
def kill_agent(db, agent_id):
    ...
    row = db.execute(
        "SELECT pid, status FROM agents WHERE id=?",
        (agent_id,
         )).fetchone()
    if not row:
...
def monitor_agents(db):
    ...
    running = db.execute(
        "SELECT id, pid, timeout_sec, started_at FROM agents WHERE status='running'").fetchall()
    results = []
    for aid, pid, timeout, started in running:
        ...
        if not alive:
            ...
            results.append({"agent_id": aid, "pid": pid,
                           "action": "marked_disappeared"})
        else:
            if started:
                try:
                    ...
                    results.append(
                        {"agent_id": aid, "pid": pid, "action": "still_running"})
            else:
                results.append({"agent_id": aid, "pid": pid,
                               "action": "still_running"})
...
def do_status(db):
    ...
    return {"script": "ia_agent_spawner.py",
            "id": 201,
            "db": str(DB_PATH),
            "total_agents": total,
            "stats": stats,
            "recent": [{"id": r[0],
                        "goal": r[1],
                        "status": r[2],
                        "created": r[3]} for r in recent],
            "ts": datetime.now().isoformat()}


def main():
    parser = argparse.ArgumentParser(
        description="IA Agent Spawner — spawn/monitor micro-agents")
    parser.add_argument("--spawn", type=str, help="Spawn agent from JSON spec")
    parser.add_argument(
        "--list",
        action="store_true",
        help="List recent agents")
    parser.add_argument(
        "--kill",
        type=int,
        metavar="ID",
        help="Kill a running agent")
    parser.add_argument(
        "--monitor",
        action="store_true",
