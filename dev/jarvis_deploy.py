...
def main():
    ...
    if not args.skip_pull:
        if args.dry_run:
            ...
            print(
                f"Git pull: {'OK' if code == 0 else 'FAIL'} - {out.strip()[:100]}")

    if not args.skip_restart:
        services = [
            ("WS",
             "taskkill /F /IM python.exe /FI \"WINDOWTITLE eq jarvis_ws\" 2>nul & cd /d /home/turbo/jarvis-linux && start /b cmd /c \"title jarvis_ws && uv run python python_ws/server.py\""),
        ]
