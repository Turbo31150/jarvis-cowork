#!/usr/bin/env python3
"""cluster_ops_runner.py -- Dynamic runner for uncabled cluster_*.py scripts.

Scans the cowork/dev directory for cluster_*.py scripts not already wired
individually in the autonomous orchestrator, then runs each one sequentially
with --once, logging results to cowork_gaps.db.

Usage:
    python cluster_ops_runner.py --once          # Run all uncabled cluster scripts
    python cluster_ops_runner.py --list          # List discovered scripts
    python cluster_ops_runner.py --dry-run       # Show plan without executing
...
ALREADY_WIRED = {
    ...
    "cluster_auto_healer.py",
    "cluster_failover_manager.py",
    "cluster_load_predictor.py",
}
...
def discover_scripts():
    """Find all cluster_*.py scripts excluding self and already-wired ones."""
    scripts = []
