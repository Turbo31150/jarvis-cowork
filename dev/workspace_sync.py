...
def check_all():
    ...
    for name in scripts:
        ...
        results.append({"name": name, "size": size, "ok": ok,
                       "flag": flag, "output": output[:60]})
    return results
...
def generate_report(results):
    ...
    lines.append(
        f"  Scripts: {total} | Working: {ok_count} | Failed: {
            total - ok_count}")
    lines.append(
        f"  Coverage: {
            round(
    ...
    for r in results:
        ...
        lines.append(f"  [{status}] {r['name']} ({size_kb}KB)" +
                     (f" — {r['flag']}" if r["ok"] else ""))
    return "\n".join(lines)
