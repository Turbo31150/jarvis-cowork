...
def scan_codebase(db):
    ...
    for row in db.execute(
            "SELECT file, line, pattern FROM features WHERE status='open'"):
            ...
            for regex, ptype in PATTERNS:
                ...
                if m:
                    ...
                    if key not in existing:
                        ...
                        priority = "high" if ptype in (
                            "FIXME", "NOT_IMPL") else "medium"
                        db.execute(
    ...
    total_open = db.execute(
        "SELECT COUNT(*) FROM features WHERE status='open'").fetchone()[0]
    db.execute(
        "INSERT INTO runs (ts, files_scanned, new_found, total_open) VALUES (?,?,?,?)",
        (time.time(),
         files_scanned,
         new_found,
         total_open))
            ...
            with urllib.request.urlopen(req, timeout=30) as resp:
                ...
                for item in reversed(data.get("output", [])):
                    if item.get("type") == "message":
                        ...
                        break
                results.append(
                    f"[{fpath}:{line}] {text[:60]} → M1: {output[:100]}")
...
def main():
    ...
    parser.add_argument(
        "--dispatch",
        action="store_true",
        help="Dispatch high-prio to cluster")
    ...
    if args.loop:
        while True:
            try:
                files, new, total = scan_codebase(db)
                print(
                    f"[{time.strftime('%H:%M')}] Scanned {files} | +{new} | Open: {total}")
                if new > 0:
                    generate_report(db)
