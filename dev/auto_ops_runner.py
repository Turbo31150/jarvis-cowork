#!/usr/bin/env python3
"""auto_ops_runner.py -- Dynamic runner for uncabled auto_*.py scripts.

Scans the cowork/dev directory for auto_*.py scripts not already wired
individually in the autonomous orchestrator, then runs each one sequentially
with --once, logging results to cowork_gaps.db.

Usage:
    python auto_ops_runner.py --once          # Run all uncabled auto scripts
    python auto_ops_runner.py --list          # List discovered scripts
    python auto_ops_runner.py --dry-run       # Show plan without executing
...
def discover_scripts():
    """Find all auto_*.py scripts excluding self and already-wired ones."""
    scripts = []
